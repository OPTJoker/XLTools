...
+ (instancetype)ShareInfoView{
    @synchronized(self){
        if (nil == infoView) {
            ...
            infoView.frame = CGRectMake((KSCRWIDTH-infoW)/2., (KSCRHEIGHT-infoH)/2., infoW, infoH);
            infoView.scaleTransform = infoView.layer.affineTransform;
            
            
            float btnS = 36.;
            UIButton *close = [UIButton buttonWithType:UIButtonTypeCustom];
            close.frame = CGRectMake((KSCRWIDTH - (KSCRWIDTH-infoW)/2.)-btnS, (KSCRHEIGHT-infoH)/2., btnS, btnS);
            ...
            [infoView->bgView addSubview:close];
...
- (void)setInfoArr:(NSArray *)infoArr{
    ...
    CGSize size = [@"技术参数" sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10]}];
    DLog(@"?????sizeH:%f",size.height);
    infoView.contentSize = CGSizeMake(infoW, 20+14+12+(11+5)*infoArr.count);
    ...
    if (infoArr!=nil) {
        ...
        for (NSDictionary *dic in infoArr) {
            ...
            lastVLab = valLab;
            // infoView.contentSize = CGSizeMake(infoW, valLab.frame.origin.y+16);
        }
